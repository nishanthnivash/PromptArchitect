  ...
  return (
    <div className="min-h-screen bg-[#fcfcfd]">
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Header Navigation */}
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 bg-white/40 backdrop-blur-md p-3 rounded-2xl border border-white/20 sticky top-4 z-50 shadow-sm ring-1 ring-slate-900/5">
          <div className="flex items-center gap-4">
            <div onClick={handleNewPrompt} className="bg-primary p-2 rounded-xl shadow-lg shadow-primary/20 cursor-pointer hover:scale-105 transition-transform">
              <Sparkles className="w-5 h-5 text-white" />
            </div>
          ...
          <div className="flex items-center gap-3">
              ...
              <div className="hidden lg:flex items-center gap-1 bg-slate-100/50 p-1 rounded-xl border border-slate-200/50">
                <Button 
                  ...
                  variant="ghost" 
                  size="sm" 
                  className="h-8 px-3 gap-2 text-[11px] font-bold text-slate-600 hover:bg-white hover:text-primary transition-all"
                >
                ...
                <Dialog>
                  <DialogTrigger asChild>
                    <Button variant="ghost" size="sm" className="h-8 px-3 gap-2 text-[11px] font-bold text-slate-600 hover:bg-white hover:text-primary">
                ...
                <Button 
                  ...
                  size="sm" 
                  className={cn("h-8 px-3 gap-2 text-[11px] font-bold transition-all", showHistory ? "bg-white text-primary shadow-sm" : "text-slate-600")} 
                  onClick={() => setShowHistory(!showHistory)}
                >
                  <History className="w-3.5 h-3.5" /> Library
                </Button>
              </div>
                    ...
                    <div className="h-full bg-white flex flex-col">
                      <div className="p-6 border-b border-slate-100">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-lg font-bold text-slate-900">PromptArchitect</span>
                        </div>
                      </div>
                      <div className="flex-1 overflow-hidden">
                        <LibraryContent 
                          ...
                          onDelete={(id) => deleteDoc(doc(db!, "savedPrompts", id))}
                        />
                      </div>
                    </div>
                  </SheetContent>
              ...
              <div className="flex items-center gap-2">
                <Avatar className="h-8 w-8 rounded-lg ring-2 ring-white shadow-sm transition-transform hover:scale-105">
                  <AvatarImage src={user.photoURL || ""} className="rounded-lg" />
                  <AvatarFallback className="rounded-lg bg-primary text-white font-bold text-xs">{user.displayName?.[0]}</AvatarFallback>
                </Avatar>
                <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-colors" onClick={handleLogout}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className={cn(
            "lg:col-span-3 space-y-6 animate-in slide-in-from-left duration-500",
            !showHistory && "hidden lg:block lg:opacity-0 lg:pointer-events-none lg:w-0 lg:overflow-hidden lg:m-0",
            "hidden lg:block"
          )}>
          ...
          <div className={cn("space-y-8 transition-all duration-500", showHistory ? "lg:col-span-9" : "lg:col-span-12")}>
            <Card className="shadow-xl border-none bg-white/60 backdrop-blur-xl overflow-hidden ring-1 ring-slate-900/5 relative group">
              <div className="h-1.5 bg-gradient-to-r from-primary via-accent to-primary w-full opacity-80" />
              <CardHeader className="pb-4 pt-8 px-8">
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-xl font-bold text-slate-900 flex items-center gap-2.5">
                      <Terminal className="w-5 h-5 text-primary" /> Engineering Workspace
                    </CardTitle>
                    <CardDescription className="text-[13px] font-medium text-slate-500 mt-1">Describe your objective. Gemini will architect the instructions.</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="px-8 pb-8 space-y-6">
                <div className="relative">
                  <Textarea
                    ...
                    className="min-h-[180px] text-lg border-none focus-visible:ring-0 bg-slate-50/30 rounded-2xl leading-relaxed placeholder:text-slate-300 transition-all hover:bg-slate-50/50 p-6 shadow-inner ring-1 ring-slate-200/50"
                    value={rawThought}
                    onChange={(e) => setRawThought(e.target.value)}
                  />
                  <div className="absolute bottom-4 right-4 text-[10px] text-slate-400 font-medium">
                    {rawThought.length} characters
                  </div>
                </div>
                
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                  {results?.detectedIntent ? (
                    <div className="flex items-center gap-4 bg-primary/[0.05] px-5 py-2.5 rounded-2xl border border-primary/10 animate-in fade-in slide-in-from-left-4 duration-500">
                      <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
                      <div>
                        <p className="text-[8px] font-bold text-primary uppercase tracking-[0.15em] mb-0.5">Detected Intent</p>
                        <p className="text-[14px] font-bold text-slate-900">{results.detectedIntent}</p>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2.5 text-slate-400 px-2">
                       <Lightbulb className="w-3.5 h-3.5 text-amber-500" />
                       <p className="text-xs font-medium italic">Be as specific as possible for better architectures.</p>
                    </div>
                  )}
                  <Button 
                    onClick={handleGenerate} 
                    disabled={loading}
                    className="bg-primary hover:bg-primary/90 text-white ml-auto h-12 px-8 rounded-xl shadow-lg shadow-primary/20 active:scale-95 transition-all font-bold text-sm"
                  >
      ...
      <div className="p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-bold flex items-center gap-2 text-slate-900">
            <History className="w-4 h-4 text-primary" /> Architecture Library
          </h2>
          <Badge variant="secondary" className="bg-slate-100 text-slate-600 border-none text-[10px] font-bold px-2">{filteredHistory.length}</Badge>
        </div>
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
          <Input 
            placeholder="Search patterns..." 
            className="pl-9 h-9 text-[11px] bg-slate-50 border-none rounded-lg focus-visible:ring-primary/20" 
            value={searchQuery} 
            onChange={e => setSearchQuery(e.target.value)} 
          />
        </div>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-2 h-8 bg-slate-50 p-0.5 rounded-lg">
            <TabsTrigger value="all" className="text-[10px] font-bold data-[state=active]:bg-white data-[state=active]:shadow-xs">Recent</TabsTrigger>
            <TabsTrigger value="starred" className="text-[10px] font-bold data-[state=active]:bg-white data-[state=active]:shadow-xs">Starred</TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
          ...
          {filteredHistory.length === 0 ? (
            ...
            filteredHistory.map((item: any) => (
              <div 
                key={item.id} 
                className={cn(
                  "p-3 rounded-xl border border-transparent hover:border-slate-100 hover:bg-slate-50/50 group relative transition-all cursor-pointer",
                  activeDocId === item.id && "bg-primary/[0.03] border-primary/10 shadow-sm"
                )}
              ...
              >
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 h-6 w-6 text-slate-400 hover:text-rose-500 transition-all" 
                ...
                <div className="flex items-center gap-2 mb-1 pr-6">
                  ...
                  <p className="text-[11px] font-bold truncate text-slate-700">{item.detectedIntent}</p>
                </div>
                <p className="text-[10px] text-slate-400 line-clamp-1 leading-relaxed italic">"{item.rawThought}"</p>
                <p className="text-[8px] text-slate-300 mt-1.5 font-bold uppercase tracking-tighter">
