...
export function PromptArchitect() {
  ...
  const handleStarToggle = (promptIdx: number) => {
    ...
    // Check if this result is from history or just generated
    // If it's a current generation, find its doc in history if possible
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className={cn("space-y-8", showHistory ? "lg:col-span-8" : "lg:col-span-12")}>
          <Card className="shadow-lg border-none bg-white overflow-hidden">
            ...
            <CardHeader>
              <CardTitle className="text-lg font-medium">Draft your prompt logic</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                >
                  ...
                  {loading ? "Architecting..." : "Generate 4 Architected Prompts"}
                </Button>
              </div>
            </CardContent>
          </Card>
        ...
        {showHistory && user && (
          <div className="lg:col-span-4 space-y-4">
            <Card className="h-[calc(100vh-200px)] shadow-xl border-none">
                ...
                <ScrollArea className="h-[calc(100vh-400px)] px-4 pb-4">
                  <div className="space-y-3">
                    {filteredHistory?.map((item) => (
                      <div key={item.id} className="p-4 border rounded-xl bg-secondary/5 group relative hover:border-primary/20 transition-colors">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 h-7 w-7 text-destructive hover:bg-destructive/10"
                        ...
                        <p className="text-[10px] text-muted-foreground line-clamp-2 mb-3 bg-white/50 p-2 rounded border border-black/5 italic">
                          "{item.rawThought}"
                        </p>
                        <Button 
                          variant="secondary" 
                    ...
                    {!filteredHistory?.length && (
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p className="text-xs text-muted-foreground">No matching prompts found.</p>
                      </div>
                    )}
                  </div>
                </ScrollArea>
